...
class GaussianHMM(_BaseHMM):
    ...
    def covars_(self):
        ...
        return fill_covars(
            self._covars_, self.covariance_type, self.n_components, self.n_features
        )

    @covars_.setter
    def covars_(self, covars):
        covars = np.array(covars, copy=True)
        _utils._validate_covars(covars, self.covariance_type, self.n_components)
    ...
    def _init(self, X, lengths=None):
        ...
        if self._needs_init("c", "covars_"):
            ...
            if not cv.shape:
                cv.shape = (1, 1)
            self.covars_ = _utils.distribute_covar_matrix_to_match_covariance_type(
...
def _multinomialhmm_fix_docstring_shape(func):
    ...
    if doc is None:
        ...
        wrapper = functools.wraps(func)(lambda *args, **kwargs: func(*args, **kwargs))
        wrapper.__doc__ = (
...
class MultinomialHMM(_BaseHMM):
    ...
    def _check(self):
        ...
        if self.emissionprob_.shape != (self.n_components, n_features):
            raise ValueError("emissionprob_ must have shape (n_components, n_features)")
        else:
...
class GMMHMM(_BaseHMM):
    ...
    def _init(self, X, lengths=None):
        ...
        main_kmeans = cluster.KMeans(n_clusters=nc, random_state=self.random_state)
        labels = main_kmeans.fit_predict(X)
        kmeanses = []
        for label in range(nc):
            kmeans = cluster.KMeans(n_clusters=nm, random_state=self.random_state)
        ...
        if self._needs_init("m", "means_"):
            self.means_ = np.stack([kmeans.cluster_centers_ for kmeans in kmeanses])
    ...
    def _fix_priors_shape(self):
        ...
        self.weights_prior = np.broadcast_to(self.weights_prior, (nc, nm)).copy()
        self.means_prior = np.broadcast_to(self.means_prior, (nc, nm, nf)).copy()
        self.means_weight = np.broadcast_to(self.means_weight, (nc, nm)).copy()

        if self.covariance_type == "full":
            self.covars_prior = np.broadcast_to(
                self.covars_prior, (nc, nm, nf, nf)
            ).copy()
            self.covars_weight = np.broadcast_to(self.covars_weight, (nc, nm)).copy()
        elif self.covariance_type == "tied":
            self.covars_prior = np.broadcast_to(self.covars_prior, (nc, nf, nf)).copy()
            self.covars_weight = np.broadcast_to(self.covars_weight, nc).copy()
        elif self.covariance_type == "diag":
            self.covars_prior = np.broadcast_to(self.covars_prior, (nc, nm, nf)).copy()
            self.covars_weight = np.broadcast_to(
                self.covars_weight, (nc, nm, nf)
            ).copy()
        elif self.covariance_type == "spherical":
            self.covars_prior = np.broadcast_to(self.covars_prior, (nc, nm)).copy()
            self.covars_weight = np.broadcast_to(self.covars_weight, (nc, nm)).copy()
    ...
    def _check(self):
        ...
        if self.weights_.shape != (nc, nm):
            raise ValueError(
                f"mixture weights must have shape "
        ...
        if self.means_.shape != (nc, nm, nf):
            raise ValueError(
                f"mixture means must have shape "
        ...
        if covars_shape != needed_shape:
            raise ValueError(
                f"{self.covariance_type!r} mixture covars must "
        ...
        if self.covariance_type == "spherical" or self.covariance_type == "diag":
            if np.any(self.covars_ < 0):
                raise ValueError(
                    f"{self.covariance_type!r} mixture covars " f"must be non-negative"
                )
            if np.any(self.covars_ == 0):
            ...
            for i, covar in enumerate(self.covars_):
                if not np.allclose(covar, covar.T):
                    raise ValueError(f"Covariance of state #{i} is not symmetric")
                ...
                if min_eigvalsh == 0:
                    _log.warning("Covariance of state #%d has a null " "eigenvalue.", i)
        elif self.covariance_type == "full":
            for i, mix_covars in enumerate(self.covars_):
                for j, covar in enumerate(mix_covars):
                    ...
                    if min_eigvalsh == 0:
                        _log.warning(
                            "Covariance of state #%d, mixture #%d "
    ...
    def _generate_sample_from_state(self, state, random_state=None):
        ...
        if self.covariance_type == "tied":
            ...
            covs = fill_covars(
                covs, self.covariance_type, self.n_components, self.n_features
            )
    ...
    def _accumulate_sufficient_statistics(
        self, stats, X, lattice, post_comp, fwdlattice, bwdlattice
    ):
        super()._accumulate_sufficient_statistics(
    ...
    def _do_mstep(self, stats):
        ...
        if "m" in self.params:
            ...
            m_n = (
                np.einsum("ijk,il->jkl", post_comp_mix, samples)
                + lambdas[:, :, None] * mus
            )
        ...
        if "c" in self.params:
            ...
            if self.covariance_type == "full":
                ...
                c_d = (stats["post_mix_sum"] + 1 + nus + nf + 1)[:, :, None, None]
